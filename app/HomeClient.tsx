...
function NewsletterPopup() {
  ...
  return (
    <div onClick={() => setShow(false)} style={{ position: 'fixed', top: 0, left: 0, width: '100%', height: '100%', background: 'rgba(0,0,0,0.6)', zIndex: 1000, display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
      <div onClick={e => e.stopPropagation()} style={{ background: '#fff', borderRadius: 16, padding: '2.5rem', maxWidth: 480, width: '90%', position: 'relative', textAlign: 'center' }}>
        <button onClick={() => setShow(false)} style={{ position: 'absolute', top: 14, right: 18, background: 'none', border: 'none', fontSize: 22, cursor: 'pointer', color: '#888' }} aria-label="Close">×</button>
        ...
        <p style={{ fontSize: 15, color: '#5a5a5a', lineHeight: 1.7, marginBottom: 20 }}>
          Join women taking control of their money. Subscribe and get a <strong>free Budget Planner</strong> delivered to your inbox.
        </p>
    ...
    <div>
      ...
      <section style={{ background: 'linear-gradient(135deg, #1B5E35 0%, #2A7A49 100%)', padding: '5rem 1.5rem', textAlign: 'center', position: 'relative', overflow: 'hidden' }}>
        ...
        <div style={{ maxWidth: 680, margin: '0 auto', position: 'relative' }}>
          <span style={{ display: 'inline-block', background: 'rgba(201,168,76,0.2)', color: '#E8C96A', fontSize: 11, fontWeight: 600, letterSpacing: '0.1em', textTransform: 'uppercase', padding: '5px 14px', borderRadius: 20, marginBottom: '1.25rem', border: '1px solid rgba(201,168,76,0.3)' }}>Women's Personal Finance</span>
          <h1 style={{ fontFamily: "'Playfair Display', serif", fontSize: 'clamp(2rem, 5vw, 3.25rem)', fontWeight: 700, color: '#fff', lineHeight: 1.2, marginBottom: '1.25rem' }}>
            Your money. Your rules.<br /><span style={{ color: '#C9A84C' }}>Your way.</span>
          </h1>
          <p style={{ fontSize: '1.05rem', color: 'rgba(255,255,255,0.8)', lineHeight: 1.75, maxWidth: 540, margin: '0 auto 2rem' }}>
          ...
          <div style={{ display: 'flex', gap: 12, justifyContent: 'center', flexWrap: 'wrap' }}>
            <Link href="/blog" style={{ background: '#C9A84C', color: '#1B5E35', padding: '13px 28px', borderRadius: 8, fontWeight: 700, fontSize: 14, textDecoration: 'none' }}>Read the Blog</Link>
            <Link href="/about" style={{ background: 'transparent', color: '#fff', padding: '13px 28px', borderRadius: 8, fontWeight: 600, fontSize: 14, textDecoration: 'none', border: '1px solid rgba(255,255,255,0.4)' }}>About Me</Link>
          </div>
        </div>
      </section>
      ...
      <section style={{ background: '#1B5E35', padding: '4rem 1.5rem', textAlign: 'center' }}>
        <div style={{ maxWidth: 560, margin: '0 auto' }}>
          ...
          <p style={{ color: 'rgba(255,255,255,0.75)', fontSize: 15, marginBottom: '1.75rem', lineHeight: 1.7 }}>
            Subscribe and get a <strong style={{ color: '#C9A84C' }}>free Budget Planner</strong> delivered straight to your inbox. Plus weekly money tips.
          </p>
